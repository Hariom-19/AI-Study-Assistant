...
- **Overfitting** — When a model memorizes training data but fails on new data`,
  ...
  quiz: [
    ...
    },
    {
      ...
      answer: "C",
      explanation:
        "Supervised Learning trains models on labeled data where the correct output is already known.",
    },
    {
      ...
      answer: "B",
      explanation:
        "Neural networks are computing systems modeled after the biological neural networks found in animal brains.",
    },
    {
    ...
    },
    {
      ...
      answer: "B",
      explanation:
        "Deep Learning uses multiple layers of neural networks to learn complex representations and patterns from data.",
    },
  ],
  ...
  flashcards: [
    {
      ...
      back: "A subset of AI that enables systems to learn and improve from experience without explicit programming.",
    },
    {
      front: "Supervised Learning",
      back: "A type of ML where the model is trained on labeled data with known correct outputs.",
    },
    {
      front: "Unsupervised Learning",
      back: "A type of ML where the model finds hidden patterns in data without labeled examples.",
    },
    {
      front: "Neural Network",
      back: "A computing system inspired by biological neural networks in the human brain.",
    },
    {
      front: "Deep Learning",
      back: "ML using multiple layers of neural networks to learn complex data representations.",
    },
    {
      front: "Training Data",
      back: "The dataset used to teach a machine learning model.",
    },
    {
      front: "Overfitting",
      back: "When a model memorizes training data too well and fails to generalize to new data.",
    },
    {
      front: "Reinforcement Learning",
      back: "ML where an agent learns by taking actions and receiving rewards or penalties.",
    },
  ],
...
const Section = ({ children, delay = 0 }) => (
  <div
    style={{ animation: `fadeUp .45s ease both`, animationDelay: `${delay}ms` }}
  >
    {children}
  </div>
);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        <div style={{ color: "var(--muted)", fontSize: 13 }}>
          PDF, TXT, or Markdown · click or drag
        </div>
        {dragging && (
          <div
            style={{
      ...
      <button
        ...
        onClick={() => {
          onTextReady(textInput, "Pasted text");
          setTextInput("");
        }}
        style={{
...
Material:\n${notes.slice(0, 6000)}`,
      ...
      setQuestions([
        {
          q: "Error generating quiz: " + e.message,
          options: {},
          answer: "",
          explanation: "",
        },
  ...
  const optColor = (qi, opt) => {
    if (!revealed[qi])
      return answers[qi] === opt ? "rgba(200,241,53,.15)" : "var(--card-bg)";
  ...
  const optBorder = (qi, opt) => {
    if (!revealed[qi])
      return answers[qi] === opt ? "var(--accent)" : "var(--border)";
          ...
          >
            {Object.entries(q.options || {}).map(([opt, val]) => (
              <button
                ...
                onClick={() => pick(qi, opt)}
                style={{
                  background: optColor(qi, opt),
                  border: `1px solid ${optBorder(qi, opt)}`,
                  borderRadius: 8,
                  padding: "10px 14px",
                  color: "var(--text)",
      ...
      {questions.length > 0 && !score && (
        <div style={{ display: "flex", gap: 10 }}>
          <button
            ...
            style={btnStyle(
              "var(--accent)",
              Object.keys(answers).length < questions.length ? 0.4 : 1,
            )}
          >
            Check Answers
          </button>
          <button
            onClick={generate}
            style={btnStyle("transparent", 1, "var(--border)", "var(--muted)")}
          >
            New Quiz
          </button>
        </div>
...
Material:\n${notes.slice(0, 6000)}`,
      ...
      setCards([{ front: "Error", back: e.message }]);
    }
    setLoading(false);
  };
      ...
      {active.length === 0 && cards.length > 0 && (
        ...
        >
          ...
          <div style={{ color: "var(--muted)", marginBottom: 16 }}>
            You've been through all {cards.length} cards.
          </div>
          <button
            onClick={() => {
              setKnown([]);
        ...
        <>
          ...
          <div
            ...
            style={{
              ...
              boxShadow: flipped
                ? "0 0 0 1px var(--accent2), 0 8px 32px rgba(74,240,196,.1)"
                : "none",
            }}
          >
            <div
            ...
            <div
              style={{
                ...
                lineHeight: 1.5,
                color: flipped ? "var(--accent2)" : "var(--text)",
...
function ExplainPanel({ notes }) {
  ...
  const explain = async () => {
    ...
    setResult("");
    const levelDesc = {
      simple: "a 10-year-old",
      teen: "a high school student",
      college: "a college student",
      expert: "a graduate student",
    };
    try {
      const text = await callClaude(
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", gap: 8 }}>
        {LEVELS.map((l) => (
          <button
            ...
            style={{
              flex: 1,
              background: level === l.id ? "var(--accent)" : "var(--panel)",
              border: `1px solid ${level === l.id ? "var(--accent)" : "var(--border)"}`,
              borderRadius: 8,
              padding: "8px 6px",
              cursor: "pointer",
              color: level === l.id ? "#0d0f14" : "var(--muted)",
              transition: "all .15s",
            }}
          >
            <div
      ...
      <div style={{ display: "flex", gap: 10 }}>
        ...
        <button
          ...
          style={btnStyle("var(--accent)", topic.trim() && !loading ? 1 : 0.4)}
        >
...
function MarkdownView({ text, onReset, resetLabel }) {
  const html = text
    ...
    .replace(
      /`(.+?)`/g,
      `<code style="background:var(--panel);padding:1px 5px;border-radius:3px;font-family:'DM Mono',monospace;font-size:.9em">$1</code>`,
    )
    .replace(
      /^### (.+)$/gm,
      `<h3 style="font-family:'Syne',sans-serif;font-size:15px;margin:18px 0 6px;color:var(--accent)">$1</h3>`,
    )
    .replace(
      /^## (.+)$/gm,
      `<h2 style="font-family:'Syne',sans-serif;font-size:17px;margin:20px 0 8px;color:var(--text)">$1</h2>`,
    )
    .replace(
      /^# (.+)$/gm,
      `<h1 style="font-family:'Syne',sans-serif;font-size:20px;margin:20px 0 10px;color:var(--accent)">$1</h1>`,
    )
    .replace(/^- (.+)$/gm, `<li style="margin:4px 0;padding-left:4px">$1</li>`)
    .replace(
      /(<li.*<\/li>\n?)+/g,
      (s) => `<ul style="padding-left:20px;margin:8px 0">${s}</ul>`,
    )
    .replace(/\n\n/g, "<br/><br/>")
    .replace(/\n/g, "<br/>");
    ...
    <div>
      ...
      {onReset && (
        <button
          onClick={onReset}
          style={{
            ...btnStyle("transparent", 1, "var(--border)", "var(--muted)"),
            marginTop: 14,
            fontSize: 12,
          }}
        >
...
function btnStyle(bg, opacity = 1, borderColor, color) {
  return {
    background: bg,
    opacity,
    border: `1px solid ${borderColor || (bg === "transparent" ? "var(--border)" : bg)}`,
    borderRadius: 8,
    padding: "10px 18px",
    color:
      color ||
            ...
            <Section>
              <div style={{ marginBottom: 28, textAlign: "center" }}>
                ...
                >
                  Drop your notes,
                  <br />
                  <span style={{ color: "var(--accent)" }}>ace your exam.</span>
                </div>
                <div
                  style={{
                    color: "var(--muted)",
                ...
                >
                  Upload any study material and get instant summaries, quizzes,
                  flashcards, and plain-English explanations.
                </div>
              </div>
              <div
                style={{
